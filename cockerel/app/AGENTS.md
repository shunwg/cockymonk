# AGENTS.md — cockerel/app

**Cockerel** — the Expo/React Native port of the same game (see
`../CLAUDE.md`), targeting iOS (TestFlight first), Android, and web from one
codebase, alongside (not replacing) the vanilla-JS web app in
`../js`/`../index.html`. Modeled directly on `ordkrig/`'s proven Expo setup.
The whole project (repo dir, web app, this native client) is branded
"Cockerel" (`app.json`'s `name`/`slug`) — the one thing that did NOT change
is the bundle identifier, still `com.edword92.dailycock` (registered on
Edvard's Apple team; changing it is an App Store Connect action, out of
scope for an agent). The Fly.io apps were previously `the-daily-cock`/
`daily-c-staging` and are now `cockerel`/`cockerel-staging`.
...
**Run locally** — points at the already-deployed `cockerel-staging` backend
by default, so no local server is needed to play a full round:
```bash
...
- `src/lib/apiConfig.ts` resolves `EXPO_PUBLIC_API_URL`, falling back to the
  **`cockerel-staging`** Fly.io app — a separate, isolated backend/volume from
  the production web app (`cockerel.fly.dev`). Don't repoint this app at
  production without an explicit decision to merge the two user pools.
...
- **Never point `EXPO_PUBLIC_API_URL`/`apiConfig.ts`'s fallback at the
  production web app** (`cockerel.fly.dev`) — staging (`cockerel-staging`)
