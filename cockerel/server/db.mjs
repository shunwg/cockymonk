 ...
 * guesses, and any Google account linked to it) — safe to expose on a shared
 * instance, since it never touches anyone else's profile or progress. A
 * bluff this user already wrote into an already-SEALED batch keeps its
 * authorship there untouched (rewriting frozen option pools would be its own
 * can of worms) — worst case a stray fooled-vote credit later recreates a
 * fresh, empty profile under the old userId, which is harmless since no
 * browser holds that identity anymore. The identity unlink matters more than
 * that: without it, a "deleted" account would quietly come back to life the
 * next time that same Google account signs in (linkGoogleIdentity would
 * resurrect the old userId) — this is the actual delete-my-account path, so
 * it must sever the link, not just clear the local data.
 */
 ...
 * anonymous profile, progress and all). Either way `ensureProfile`'s
 * won't-overwrite-an-existing-displayName rule applies unchanged, so linking
 * never silently renames a profile to the Google account's real name.
 */
export function linkGoogleIdentity(db, { sub, userId, displayName, device }) {
  ...
  ensureProfile(db, finalUserId, displayName, device);
  return {
    userId: finalUserId,
    profile: profileSnapshot(db, db.profiles[finalUserId]),
...
/**
 * Full reset back to a brand-new-deployment state: every profile,
 * submission, guess, day-result, and Google identity link, plus the batch
 * history, dev clock, and cached bot leaderboard. The next `ensureToday`
 * bootstraps fresh exactly like a first-ever boot (see that function's
 * bootstrap branch) — this is NOT a per-user reset (see `resetPlayer`), it's
 * for the one-time "require sign-in from now on, wipe everyone who played
 * anonymously before that" cutover. Gated behind ADMIN_TOKEN in
 * dev-server.mjs, same posture as the rest of the admin surface.
 */
