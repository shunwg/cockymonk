...
- No networking beyond local MultipeerConnectivity — if you find yourself importing URLSession, stop.
...
## Lanes (3-person parallel work)
Ownership matrix and interface contracts live in `LANES.md`. Summary: **Lane A** = Engine + Transport (pure Swift + `Lab/js/engine.js` + `Tools/engine-vectors.json`) · **Lane B** = UI + Board + Themes + Motion (`Lab/`, `Sources/Views|Themes`, tokens via /director) · **Lane C** = Content + Decks + Audio + Assets (`Resources/deck_*`, `Content/`, ASSETS.md rows). Cross-lane edits go through `/director`. Each lane has a standalone test that runs on Windows — keep it green before handing work across lanes.
